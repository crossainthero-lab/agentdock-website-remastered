...
import { Download, Terminal, Copy, Check, ExternalLink, AlertCircle } from 'lucide-react';
import { motion } from 'motion/react';
import { fetchReleases, ReleasesData } from '../lib/releases';
import { DownloadOption, currentDownloads } from '../config/downloads';
...
npm run build`;
  ...
  const renderCurrentAsset = (d: DownloadOption, i: number, delayOffset: number = 0) => (
    <motion.div 
      ...
      <div className="flex justify-between items-start mb-4 relative z-10">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            {!d.recommended && d.version.includes('Preview') && (
              <span className="px-2 py-0.5 text-[10px] font-bold text-[var(--color-accent-purple)] bg-[var(--color-accent-purple-soft)] border border-[var(--color-accent-purple-border)] rounded-full tracking-wide uppercase">Preview</span>
            )}
          </div>
  ...
  return (
    ...
    >
      ...
      {error ? (
        <div className="p-6 border border-red-900/50 bg-red-900/10 rounded-xl mb-12 text-center">
          <AlertCircle className="w-8 h-8 text-red-500 mx-auto mb-3" />
          <h3 className="text-lg font-bold text-white mb-2">Could not fetch releases</h3>
          <p className="text-red-200/70 text-sm mb-4">We are having trouble connecting to GitHub right now. You can view and download all releases directly from our GitHub repository.</p>
          <a href="https://github.com/crossainthero-lab/AgentDock/releases" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-6 py-2.5 text-sm font-bold text-white bg-[var(--color-accent-purple)] hover:bg-[var(--color-accent-purple-hover)] rounded-md transition-all">
            <ExternalLink className="w-4 h-4" /> View GitHub Releases
          </a>
        </div>
      ) : loading ? (
          ...
          <section>
            ...
            {windows.length > 0 ? (
              ...
              <div className="p-6 border border-[var(--color-ad-border)] bg-[var(--color-ad-surface)] rounded-xl">
                <p className="text-[var(--color-ad-text-muted)] text-sm font-medium">No Windows releases available at the moment. Please build from source.</p>
              </div>
            )}
          </section>
